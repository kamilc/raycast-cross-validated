...
export const searchResources = async (q: string): Promise<QueryResultItem[]> => {
  ...
  const query = `https://api.stackexchange.com/2.3/search/advanced?key=U4DMV*8nvpm3EOpvf69Rxw((&site=crossvalidated&page=1&pagesize=20&order=desc&sort=relevance&q=${encodeURIComponent(
    qss
